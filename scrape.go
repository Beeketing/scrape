...
// ByAttr returns a Matcher which matches all nodes with the provided id.
func ByAttr(key string, values string) Matcher {
	return func(node *html.Node) bool { return Attr(node, key) == values }
}

// CombineMachers ----
//Using for combine one or more matcher by AND CLAUSE in scrape.---
func CombineMachers(listMachers []Matcher) Matcher {
...
// HasAttr returns a Matcher which matches all nodes with the provided class.
func HasAttr(attr string) Matcher {
	return func(node *html.Node) bool {
		for _, a := range node.Attr {
			if a.Key == attr {
				return true
...
// FindFirstInternal encapsulates the node tree traversal
func FindFirstInternal(node *html.Node, matcher Matcher) *html.Node {
	...
	for c := node.FirstChild; c != nil; c = c.NextSibling {
		found := FindFirstInternal(c, matcher)
		if found != nil {
			return found
		}
	}
